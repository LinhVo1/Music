...
def extract_features(y, sr=40000, nfilt=10,winsteps=0.02):
    try:
...
for namesong in tqdm(os.listdir(opt.test_folder)):
    ...
    y, sr = librosa.load(namesong, sr=40000)
    ...
    for i in range(0,opt.n):
        file.writelines(most_song.most_common()[i][0][length+1:]+'->'+str(most_song.most_common()[i][1])+'\n')
    file.writelines('\n')
...
# python find_song_40.py --train_folder train/ --test_folder test/  --n 1 --f 100 --nfilt 10
