...
# Lap trong thu muc data/wav/train va tao ra 200 file anh spectrogram
Data_dir=np.array(glob(wav_path+"train/*"))

for file in Data_dir[0:200]:
    filename,name = file,file.split('/')[-1].split('.')[0]
    create_spectrogram(filename,name, train_data_path)

gc.collect()

# Lap trong thu muc data/wav/test va tao ra 30 file anh spectrogram
Test_dir=np.array(glob(wav_path+"test/*"))


for file in Test_dir[0:200]:
    filename,name = file,file.split('/')[-1].split('.')[0]
